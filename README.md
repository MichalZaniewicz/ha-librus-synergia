...
is a companion HACS repo with 53 purpose-built cards - grade averages, a full
...
| `sensor` | Homework assignments | Count of real "zadania domowe" (distinct from the Agenda calendar's general feed below), with a `recent` attribute (id/topic/text/due date/teacher) |
...
- **Grade value parsing**: real numeric grades (e.g. a plain `4` or `6`) and non-numeric marks (e.g. a bare `+` for "aktywność") are CONFIRMED live to parse and average correctly - a non-numeric mark is correctly excluded from the average rather than crashing or counting as 0. `+`-modified grades have since appeared too (e.g. `4+`, `5+`) and average out to `+0.5` as expected - but that only shows the code is self-consistent, not that `+0.5`/`-0.25` is the value Librus itself intends, since the average is computed with that same assumed convention. Still a common third-party inference, not something Librus documents anywhere. No `-`-modified grade has appeared yet to check the `-0.25` half at all.
